...
export class StorageService {
  ...
  public static async getAllPhotos(): Promise<Photo[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onsuccess = () => {
        ...
        photos.sort((a, b) => (b.dateAdded || 0) - (a.dateAdded || 0));
        resolve(photos);
      };
  ...
  public static async createAlbum(title: string, description?: string): Promise<Album> {
    const db = await this.getDB();
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        ...
        img.onload = async () => {
          ...
          const dateTaken = new Date(file.lastModified).toISOString().split('T')[0];

          const extractedExif = await ExifExtractor.extractMetadata(file);
